...
通过captcha模块生成一个验证码生成器，然后训练CNN模型破解自己做的验证码生成器。
'''

from gcapttcha import gen_captcha_text_and_image
from gcapttcha import number, alphabet, ALPHABET
import numpy as np
...
print "验证码最长字符数", MAX_CAPTCHA


# 把彩色图像转换为灰度图像
def conver2gray(img):
    if len(img.shape) > 2:
        gray = np.mean(img, -1)     # Gray = (R+G+B)/3
        # 上面的转换方法较快, 正规转法:
        # r, g, b = img[:,:,0], img[:,:,1],img[:,:,2]
        # gray = 0.2989*r + 0.5870*g+ 0.1140*b        # RGB转换成灰度的算法
        return gray
    else:
        return img


char_set = number + alphabet + ALPHABET + ['_']    # 如果验证码长度小于4, 用 "_"补齐
