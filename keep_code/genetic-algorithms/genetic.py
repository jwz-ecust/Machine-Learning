...
class GA():
    ...
    def evolve(self, retain_rate=0.2, random_select_rate=0.5, mutation_rate=0.01):
        '''
        进化
        对当前一代种群依次进行选择,交叉生成新的一代种群,然后对新的一代种群进行变异
        '''
    ...
    def gen_chromosome(self, length):
        '''
        随机生成长度为length的染色体,每个基因的取值为0or1
        这里用一个bit表示一个基因
        '''
        chromosome = 0
    ...
    def selection(self, retain_rate, random_select_rate):
        '''
        选择
        先对适应度从打到小排序, 选出存活的染色体
        再进行随机选择, 选出适应度虽然小,但是存活下来的个体
        '''
        # 对适应度从小到大进行排序
    ...
    def crossover(self, parents):
        ...
        染色体的交叉, 繁殖, 生成新一代的种群
        '''
        # 新出生的孩子,最终呗加入存活下来的父母之中, 形成新一代的种群
        children = []
        # 需要繁殖的孩子的量
        target_count = len(self.population) - len(self.parents)
        # 开始根据需要的量进行繁殖
        while len(children) < target_count:
            ...
            if male != female:
                ...
                # 孩子将获得父亲在交叉前的基因和母亲在交叉后(包括交叉点)的基因
