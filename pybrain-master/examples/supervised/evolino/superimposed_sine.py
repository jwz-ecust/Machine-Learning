...
from pylab import plot, figure, show, ion, cla, subplot, title, figlegend, draw
import numpy
...
for i in range(1000):
    ...
    trnSequenceOutput = net.extrapolate(
        trnSequenceWashout, len(trnSequenceTarget))
    tstSequenceOutput = net.extrapolate(
        tstSequenceWashout, len(tstSequenceTarget))

    # plot training data
    fig = figure()

    sp = fig.add_subplot(2, 1, 1)  # switch to the first subplot
...
fig.savefig(
    '/Users/zhangjiawei/Code/Machine-Learning/pybrain-master/examples/supervised/evolino/zjw.jpg')
